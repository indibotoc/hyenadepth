...
def evaluate(conf):
    ...
    if conf['numpy_disparities_to_evaluate'] is not None: # optional path to load disparities from a .npy file for evaluation - no model inference.
        ...
        # Preparing model
        if conf['model_name'] == "hyena":
            model = HyenaDepth(pretrained=conf['hyena']['pretrained'], scales=conf['hyena']['scales'])
            model.from_pretrained(encoder_weights_path=conf['hyena']['encoder_weights_path'], decoder_weights_path=conf['hyena']['decoder_weights_path'], weights_path=conf['hyena']['weights_path'], device=device)
